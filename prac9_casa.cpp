...
GRUPO:02
PRACTICA 9 (CASA)
MESA DE AJEDREZ CON TEXTURA DE MADERA Y BASE DE METAL AGREGANDO UNA SILLA APLICANDO TRANSPARENCIA  
*/
...
void silla()
{
	glPushMatrix();
		...
		glBindTexture(GL_TEXTURE_2D, chair.GLindex);
		glPushMatrix();
			glRotatef(105, 1, 0, 0);
...
void InitGL(GLvoid)     // Inicializamos parametros
{
	...
	chair.LoadTGA("silla.tga");
	chair.BuildGLTexture();
	chair.ReleaseImage();
}
...
void display(void)   // Creamos la funcion donde se dibuja
{
   ...
   //Borde de la izquierda
	...
	glPushMatrix();
		glTranslatef(9.5, 4.5, -9.5);
		silla();
	glPopMatrix();

	glPopMatrix();
	glDisable(GL_TEXTURE_2D);
	renderBitmapCharacter(-11, 12.0, -14.0, (void *)font, "Practica 9 (casa)");
	renderBitmapCharacter(-11, 10.5, -14, (void *)font, "MESA DE AJEDREZ Y SILLA (transparencia)");
	glEnable(GL_TEXTURE_2D);
